...
export const TotalBudgetCard = () => {
  ...
  const amount = expenses.reduce((total:number, expense:Expenses) => total + expense.amount, 0);
  const max = budgets.reduce((total:number, budget:Budgets) => total + budget.max, 0);
